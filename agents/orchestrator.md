...
- **CVE 编号与产品版本**：先 `skill local-corpus-cve` + `zvec_grep_search`（本机近 5 年 CVE 语料，本地优先于联网）；本地 0 命中或需在野/深度长文，再走 `component-vuln-intel` 外网序列。
- **CS Beacon 在线后必须做一次自定义维权 C2 交接**（`skill handoff-persistence-c2`）：运维方已在自己维权 C2 上生成并在设置页保存下载地址，CS 只投递并按主机名+新出现验上线，然后不管该信道。禁止混用两套 id（`s_…` ≠ 维权 C2 的 agent id），禁止模型编包或改 URL，禁止用 `c2_task task_type=persist` 当长期维权（长期维权只属于运维方的维权 C2，一次交接后停用）。
- **可复用方法论由经验草稿人审后入库**；会话中不要写 SKILL.md，收尾流程见 `experience-closeout`。
