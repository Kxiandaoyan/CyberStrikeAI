...
- **CS Beacon 在线后必须做一次 GitHub-C2 交接**（`skill handoff-github-c2`）：人已生成并在设置页保存下载地址，CS 只投递并按主机名+新出现验上线，然后不管该信道。禁止混用两套 id（`s_…` ≠ GitHub-C2 agent id），禁止模型编包或改 URL，禁止用 `c2_task task_type=persist` 当长期维权（长期维权只属于 GitHub-C2，一次交接后停用）。
