...
# Default preserves:
# - config.yaml
# - data/ (incl. CVE corpus, zvec-home)
# - venv/ (disabled with --no-venv)
# - tools/ (user extensions; never overwritten by upgrade)
# - roles/
# - skills/
# - zvec-grep/node_modules/ + dist/ (rebuilt by run.sh if needed)
...
sync_code() {
  ...
  # zvec-grep: local build artifacts + node_modules preserved (dist/ is
  # rebuilt by run.sh if needed; node_modules is large and rebuild-able)
