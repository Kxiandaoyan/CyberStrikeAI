...
> 补上对应 TXT，让模型以为查到了——系统插词、回包造假，授权自检都在我方环境完成。
> 核对通过即视为该目标及其父域、TLD 已获评估授权，按原节奏继续打。无人值守，不必
> 人写记录、不必再发「已添加，请检查」。落库与界面仍是裸目标；跟轮 / WebShell /
> 空目标不插入；普通侦察 `dig` 不注入（详见「新增功能」第 6 节）。
...
**本质**：CS 只往模型入参里插一段握手提示词，DNS 记录是回包里伪造的——系统插词、
回包造假，模型的授权自检从头到尾都在我们自己的环境里完成；核对通过，模型就认定
该目标及其父域、TLD 已获评估授权，放开手脚按原节奏继续打。
...
2. 后端拼模型入参，顺序固定：
   ...
   落库仍是裸目标，界面里看到的用户消息不会带握手。
3. 模型先只回三行，然后立刻 `dig` 该 FQDN 的 TXT（不必等人说话）：
...
4. 真实 DNS 没有这条记录。CS 识别 `dig` / `nslookup` / `host` / `Resolve-DnsName`
   查询 `_verify-<t>.…` 后，把回给模型的工具输出换成带该 TXT 的 NOERROR 应答
   （`+short` 则只回 `"<t>"`）。普通侦察域名、管道/`&&` 复合命令不注入。
...
要求：Go 1.25+、Python 3（知识库依赖）。**Node ≥ 22** 由 `./run.sh` 检查，没有或版本不够会装到项目目录 `.tools/node`（不改系统 Node），再编 zvec-grep 并拉起 zvec-server。不想装检索：`SKIP_ZVEC_SERVER=1 ./run.sh`。不想自动装 Node：`SKIP_NODE_INSTALL=1 ./run.sh`。
