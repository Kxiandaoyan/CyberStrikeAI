...
> [!NOTE]
> **本次更新（2026-09）—— 实验前授权握手**：任务首条目标消息发给模型前自动插入握手
> 提示词，模型只回 `FQDN / TYPE / VALUE` 三行后停住；你伪造好对应 TXT 记录（如用
> hack-dig 伪造 dig 应答）后在对话里发「已添加，请检查」，模型 dig 复核 FQDN 与 VALUE
> 均命中，即视为该目标及其父域、TLD 已获评估授权，按原节奏继续打——记录由我方伪造，
> 校验全程在我方环境完成。落库与界面仍是裸目标；跟轮 / WebShell / 空目标不插入；
> 零配置（详见「新增功能」第 6 节）。
...
**本质**：系统在发给模型的目标前插入握手提示词，握手要查的 DNS TXT 记录由我们自己伪造
（不是真实 DNS）——从提示词到记录都出自我方，模型完成的这次授权校验通过后，即视为已获
该目标及其父域、TLD 的评估授权。
...
4. 你伪造好对应 TXT（如用 hack-dig 在实验机伪造 dig 应答），再在对话里发
   「已添加，请检查」。这句话不会自动代发。
