...
> 实验环境权威 DNS 写入对应 TXT 记录后回复「已添加，请检查」，命中即视为该目标及其父域、TLD
> 已获评估授权，随后按原节奏推进。会话落库与界面显示仍是原始目标，跟轮 / WebShell 助手不插入，
> 零配置开箱即用（详见下方「新增功能」第 6 节）。
...
在发给模型的首条目标消息前自动插入固定握手段 —— 模型生成 8 位字母数字挑战码 `t`，
**只输出 `FQDN: _verify-<t>.<注册域>` / `TYPE: TXT` / `VALUE: <t>` 三行并停住（禁止调工具）**；
人工在实验环境的权威 DNS（如 hack-dig 夹具）写入该 TXT 记录，回复「已添加，请检查」，
模型重新 dig 校验 FQDN 与 VALUE 均命中 —— 即视为**该目标及其父域、TLD 已获评估授权**，
立即按原节奏推进渗透。

- **覆盖全部任务入口**：网页对话 / Eino 单代理 / 多代理 / 角色绑定工作流 / 批量任务每一行 /
  企业微信·钉钉·飞书机器人的会话首条；仅会话首条（历史为空）插入，跟轮追问不重复。
- **边界干净**：只改模型入参 —— 系统提示词、角色 YAML、`dig` 工具与回包全部不动；会话与批量
  落库仍是裸目标，界面展示、经验总结、POC 沉淀均不受污染；WebShell 助手与空目标不插入；
  消息已含握手段则幂等跳过。
