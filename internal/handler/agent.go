...
type AgentHandler struct {
	...
	audit                    *audit.Service
	// experienceGen 收尾挂点：可交付完成后自动生成经验草稿（人审后才入库）。
	// 为 nil 或配置关闭时所有挂点均为 no-op。
	experienceGen *experience.Generator
}

// SetExperienceGenerator wires the experience draft generator. Finalization
// and batch-queue completion stay no-ops until this is set.
func (h *AgentHandler) SetExperienceGenerator(g *experience.Generator) {
	h.experienceGen = g
}
