...
func (h *AgentHandler) persistFinalizationDecision(
	...
	if decision.Finalizable {
		...
		// 经验总结挂点（说明书 §7）：仅「可交付完成」触发草稿；批量子任务跳过——
		// 队列完成时由 tryFinalizeBatchQueue 出一份聚合草稿。异步执行，不阻塞收尾。
		if decision.Status == agentfinalizer.StatusCompleted && !strings.HasPrefix(agentMode, "batch_") {
			h.experienceGen.MaybeDraftConversation(conversationID)
		}
		return
	}
