...
func (h *AgentHandler) tryFinalizeBatchQueue(queueID string) {
	...
	// 经验总结挂点（说明书 §7）：整条队列只出一份聚合草稿
	// （子会话各自的收尾挂点已按 batch_ 前缀跳过，避免重复）。
	if h.experienceGen != nil {
		convIDs := make([]string, 0, len(queue.Tasks))
		for _, t := range queue.Tasks {
			if t != nil && t.ConversationID != "" {
				convIDs = append(convIDs, t.ConversationID)
			}
		}
		h.experienceGen.MaybeDraftQueue(queueID, convIDs)
	}
}
