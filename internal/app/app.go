...
type App struct {
	...
	// CVE 语料同步（cvesync.Loop 的取消函数与配置；nil = 未启用）
	cveSyncCancel context.CancelFunc
	cveSyncCfg    *cvesync.Config
	// 语料仓绝对路径（相对路径按配置文件目录解析，说明书 §4）
	corpusDirAbs string
	// 经验草稿批准后的落盘回调（skill:<name> 或 knowledge）
	experienceApplier func(d *experience.Draft, appliedTo string) (string, error)
}
...
func New(cfg *config.Config, log *logger.Logger, configPath string) (*App, error) {
	...
	// zvec-grep 自动登记：若 enabled 且 auto_register，合并到 external_mcp.servers
	// （不覆盖用户已配置的同名条目）。zvec 进程（server run + 首次索引）由
	// run.sh 管理；启动后 zvec_grep_search 的 autoUpdate 会在检索时增量索引。
	if cfg.ZvecGrep.Enabled && cfg.ZvecGrep.AutoRegister {
		zvecURL := "http://" + cfg.ZvecGrep.Listen + "/mcp"
		if cfg.ZvecGrep.Listen == "" {
			zvecURL = "http://127.0.0.1:7999/mcp"
		}
		if cfg.ExternalMCP.Servers == nil {
			cfg.ExternalMCP.Servers = make(map[string]config.ExternalMCPServerConfig)
		}
	...
	// CVE 语料仓路径：相对路径按配置文件目录解析（说明书 §4，与 roles_dir 一致），
	// 避免「从别的工作目录启动 CS 找不到语料」。
	corpusDirAbs := cfg.ZvecGrep.CorpusDir
	if corpusDirAbs == "" {
		corpusDirAbs = "data/corpus"
	}
	if !filepath.IsAbs(corpusDirAbs) {
		corpusDirAbs = filepath.Join(configDir, corpusDirAbs)
	}
	...
	// 语料仓绝对路径（路由与 cvesync 共用）
	app.corpusDirAbs = corpusDirAbs

	// CVE 语料每日同步（说明书 §3 第 6 步 / §5）：真增量（commits+compare 纯 HTTP），
	// 失败只写 last_error、不推进水位线；索引增量由 zvec 检索时 autoUpdate 完成。
	if cfg.CVECorpus.Enabled {
		syncCfg := cvesync.Config{
			CorpusDir:     corpusDirAbs,
			RawDir:        filepath.Join(corpusDirAbs, "raw", "cvelistV5"),
			Mode:          cfg.CVECorpus.Mode,
			Years:         cfg.CVECorpus.Years,
			SkipRejected:  cfg.CVECorpus.SkipRejected,
			SkipReserved:  cfg.CVECorpus.SkipReserved,
			DescMaxRunes:  cfg.CVECorpus.DescMaxRunes,
			SyncHourLocal: cfg.CVECorpus.SyncHourLocal,
			MinYearFiles:  cfg.CVECorpus.MinYearFiles,
			OnSyncDone:    handler.InvalidateCorpusCache,
		}
		cveSyncCtx, cveSyncStop := context.WithCancel(context.Background())
		app.cveSyncCancel = cveSyncStop
		cfgCopy := syncCfg
		app.cveSyncCfg = &cfgCopy
		go cvesync.Loop(cveSyncCtx, syncCfg, log.Logger.Sugar())
		log.Logger.Info("cvesync: 每日 CVE 增量同步已启动",
			zap.String("corpus", corpusDirAbs),
			zap.Int("sync_hour_local", cfg.CVECorpus.SyncHourLocal))
	}
...
func (a *App) Shutdown() {
	...
	// 停止 CVE 语料同步循环（进行中的同步由锁文件 2h 过期兜底）
	if a.cveSyncCancel != nil {
		a.cveSyncCancel()
		a.cveSyncCancel = nil
	}
...
func setupRoutes(
	...
	}))
	{
		...
		if err := experience.EnsureSchema(app.db.DB); err != nil {
			app.logger.Warn("初始化经验草稿表失败", zap.Error(err))
		} else {
			expH := experience.NewHandler(app.db.DB, app.logger.Logger.Sugar())
			expH.SetApplier(app.experienceApplier)
			expRoutes := protected.Group("/experience")
			{
				expRoutes.GET("/drafts", expH.ListDrafts)
				expRoutes.POST("/drafts", expH.CreateDraft)
				expRoutes.GET("/drafts/:id", expH.GetDraft)
				expRoutes.POST("/drafts/:id/approve", expH.ApproveDraft)
				expRoutes.POST("/drafts/:id/reject", expH.RejectDraft)
				expRoutes.GET("/stats", expH.Stats)
			}
			app.logger.Info("Experience draft API registered")
		}
