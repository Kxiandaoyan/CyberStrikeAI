...
func Load(path string) (*Config, error) {
	...
	// 经验草稿 auto_apply 锁死 false：入库只能由人在草稿页点批准，
	// 任何 yaml 写法都不允许自动应用（说明书 §7）。
	cfg.Experience.AutoApply = false
...
type CVECorpusConfig struct {
	...
	MinYearFiles  int    `yaml:"min_year_files,omitempty" json:"min_year_files,omitempty"`
	DescMaxRunes  int    `yaml:"desc_max_runes,omitempty" json:"desc_max_runes,omitempty"`
}
...
type ExperienceLLMConfig struct {
	APIKey  string `yaml:"api_key,omitempty" json:"api_key,omitempty"`
	BaseURL string `yaml:"base_url,omitempty" json:"base_url,omitempty"`
...
func (g *PersistenceC2EmbedConfig) Apply(u *PersistenceC2HandoffUpdate) error {
	...
	if u.PayloadURL != nil {
		url := strings.TrimSpace(*u.PayloadURL)
		if url != "" {
			if !strings.HasPrefix(url, "http://") && !strings.HasPrefix(url, "https://") {
				return fmt.Errorf("payload_url must start with http:// or https://")
			}
			if strings.ContainsAny(url, " \n\r\t") {
				return fmt.Errorf("payload_url must not contain whitespace")
			}
			if len(url) > 2048 {
				return fmt.Errorf("payload_url too long (max 2048)")
			}
		}
		g.PayloadURL = url
	}
	if u.DropPath != nil {
		dp := strings.TrimSpace(*u.DropPath)
		if strings.Contains(dp, "..") {
			return fmt.Errorf("drop_path must not contain ..")
		}
		g.DropPath = dp
	}
	if u.WaitSeconds != nil {
		if *u.WaitSeconds < 60 || *u.WaitSeconds > 3600 {
			return fmt.Errorf("wait_seconds must be 60-3600")
		}
		g.WaitSeconds = *u.WaitSeconds
	}
	if u.Listen != nil {
		l := strings.TrimSpace(*u.Listen)
		if l != "" && !validHostPort(l) {
			return fmt.Errorf("listen must be host:port (e.g. 127.0.0.1:8082)")
		}
		g.Listen = l
	}
	if u.WebUser != nil {
		w := strings.TrimSpace(*u.WebUser)
		if len(w) > 255 {
			return fmt.Errorf("web_user too long (max 255)")
		}
		g.WebUser = w
	}
	if u.WebPass != nil {
		p := strings.TrimSpace(*u.WebPass)
		if len(p) > 255 {
			return fmt.Errorf("web_pass too long (max 255)")
		}
		g.WebPass = p
	}
	return nil
...
// validHostPort 校验 host:port 形式（控制器地址）。host 允许 IP/域名，port 必须 1-65535。
